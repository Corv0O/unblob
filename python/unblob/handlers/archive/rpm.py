...
from .cpio import PortableASCIIParser, StrippedCPIOParser
...
class RPMParser:
    ...
    def build_stripped_entries(self) -> list[tuple[Path, int, int, str | None, int]]:
        ...
        return [
            (
                Path(dirnames[dirindexes[i]] + file_names[i]),
                modes[i],
                file_sizes[i],
                links[i] or None,
                rdevs[i],
...
class RPMExtractor(Extractor):
    def extract(self, inpath: Path, outdir: Path) -> ExtractResult:
        ...
        with File.from_path(inpath) as file:
            ...
            parser.parse()
            stripped_entries = (
                parser.build_stripped_entries() if parser.has_stripped_payload else None
            )
            file.seek(parser.payload_offset, io.SEEK_SET)
            with RPMExtractor.open_payload_stream(file, parser.compressor) as decoder:
                if stripped_entries is not None:
                    StrippedCPIOParser.extract(decoder, fs, stripped_entries)
                else:
